...
class TestCandidateRepository:
    async def test_get_by_id_returns_candidate_when_exists(self, db):
        ...
        cid = await db.add_candidate(
            item_code="REPO01", source_site="amazon", title_jp="Repo商品",
            title_en="Repo Product", cost_jpy=3000, ebay_price_usd=80.0,
            net_profit_jpy=5000, margin_rate=1.5,
        )
    ...
    async def test_list_returns_all(self, db):
        ...
        await db.add_candidate(
            item_code="LIST01", source_site="amazon", title_jp="一覧1",
            title_en=None, cost_jpy=1000, ebay_price_usd=30.0,
            net_profit_jpy=1000, margin_rate=1.0,
        )
        await db.add_candidate(
            item_code="LIST02", source_site="rakuten", title_jp="一覧2",
    ...
    async def test_list_filters_by_status(self, db):
        repo = CandidateRepository(db)
        cid = await db.add_candidate(
            item_code="FILT01", source_site="amazon", title_jp="フィルタ",
            title_en=None, cost_jpy=1000, ebay_price_usd=30.0,
            net_profit_jpy=1000, margin_rate=1.0,
        )
    ...
    async def test_count_by_status(self, db):
        ...
        await db.add_candidate(
            item_code="CNT01", source_site="amazon", title_jp="カウント",
            title_en=None, cost_jpy=1000, ebay_price_usd=30.0,
            net_profit_jpy=1000, margin_rate=1.0,
        )
    ...
    async def test_update_status(self, db):
        repo = CandidateRepository(db)
        cid = await db.add_candidate(
            item_code="UPD01", source_site="amazon", title_jp="更新",
            title_en=None, cost_jpy=1000, ebay_price_usd=30.0,
            net_profit_jpy=1000, margin_rate=1.0,
        )
...
class TestOrderRepository:
    async def test_get_by_id_returns_order_when_exists(self, db):
        ...
        oid = await db.add_order(
            ebay_order_id="ORD-001", sale_price_usd=50.0,
            buyer_username="buyer1", destination_country="US",
        )
...
class TestMessageRepository:
    async def test_get_by_id_returns_message_when_exists(self, db):
        ...
        mid = await db.add_message(
            buyer_username="msg_buyer", body="Hello",
        )
