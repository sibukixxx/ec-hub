...
## 残課題
- `api.py` の `lifespan` では `AppContext` しか初期化しておらず、APScheduler は起動していない
- `settings.scheduler` は型定義済みだが、実際のジョブ登録処理へつながっていない
- `/api/scheduler/status` と `/api/scheduler/trigger/{job_name}` は未実装
